    ...
    <motion.div key="staff_overview" initial={{ opacity: 0, scale: 0.96, filter: "blur(8px)" }} animate={{ opacity: 1, scale: 1, filter: "blur(0px)" }} exit={{ opacity: 0, scale: 1.04, filter: "blur(8px)" }} transition={{ duration: 0.4, ease: "easeOut" }} className="flex flex-col gap-6 w-full h-full">
      ...
      {tab === "attach_builder" ? (
        ...
        <div className="space-y-6">
          <LeaderboardTab staffAttachMatrix={staffAttachMatrix} parsedOfficers={parsedOfficers} formatOfficerShortName={formatOfficerShortName} matchesOfficer={matchesOfficer} />
...
function LeaderboardTab({ staffAttachMatrix, parsedOfficers, formatOfficerShortName, matchesOfficer }: { staffAttachMatrix: AttachMatrixRow[]; parsedOfficers: OfficerReport[]; formatOfficerShortName: (name: string) => string; matchesOfficer: (a: string, b: string) => boolean; }) {
  return <div className="bg-white/10 backdrop-blur-md rounded-[2rem] border border-white/10 p-6 shadow-[0_8px_32px_rgba(0,0,0,0.12)] overflow-hidden"><div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">{[{ title: "🛡️ Cover+ tgt25%", key: "Cover+", threshold: 25 }, { title: "🍎 AC+ tgt20%", key: "AC+", threshold: 20 }, { title: "✏️ Pencil tgt85%", key: "Pencil", threshold: 85 }, { title: "📱 Case tgt60%", key: "Case", threshold: 60 }].map((board) => <div key={board.key} className="bg-white/10 backdrop-blur-md rounded-2xl p-4 border border-white/10 shadow-[0_8px_32px_rgba(0,0,0,0.12)]"><span className="text-xs font-bold text-white/50 uppercase tracking-widest block mb-3 border-b border-white/5 pb-2">{board.title}</span><div className="space-y-2.5">{staffAttachMatrix.map((row) => ({ name: row.name, rate: row.attachMap[board.key]?.rate || 0 })).sort((a, b) => b.rate - a.rate).slice(0, 5).map((p, idx) => <div key={idx} className="flex justify-between items-center text-xs"><span className="text-white/80 font-medium truncate max-w-[120px]">{idx === 0 ? "🥇" : idx === 1 ? "🥈" : idx === 2 ? "🥉" : `${idx + 1}`} {formatOfficerShortName(p.name)}</span><span className={`font-bold ${p.rate >= board.threshold ? "text-emerald-400" : "text-white"}`}>{Math.round(p.rate)}%</span></div>)}</div></div>)}</div><div className="overflow-x-auto"><table className="w-full text-left text-xs border-collapse"><thead><tr className="border-b border-white/10 text-white/50"><th className="py-3 pr-4 font-bold text-center">#</th><th className="py-3 px-3 font-bold">Staff</th><th className="py-3 px-3 font-bold text-right">Target</th><th className="py-3 px-3 font-bold text-right">Actual</th><th className="py-3 px-3 font-bold text-center">Ach%</th><th className="py-3 px-3 font-bold text-right">Forecast</th><th className="py-3 px-3 font-bold text-center">C+ (25%)</th><th className="py-3 px-3 font-bold text-center">AC+ (20%)</th><th className="py-3 px-3 font-bold text-center">Pen (85%)</th><th className="py-3 px-3 font-bold text-center">Case (60%)</th><th className="py-3 px-3 font-bold text-center">SIM</th></tr></thead><tbody>{staffAttachMatrix.map((staff, idx) => { const officerState = parsedOfficers.find((o) => matchesOfficer(o.name, staff.name)); const target = officerState?.target || 0; const actual = officerState?.actual || 0; const achPercent = officerState?.achPercent || 0; const forecast = officerState?.forecast || 0; const coverPlusRate = Math.round(staff.attachMap["Cover+"]?.rate || 0); const acRate = Math.round(staff.attachMap["AC+"]?.rate || 0); const penRate = Math.round(staff.attachMap["Pencil"]?.rate || 0); const caseRate = Math.round(staff.attachMap["Case"]?.rate || 0); const simUnits = staff.attachMap["SIM"]?.units || 0; const rankEmoji = idx === 0 ? "🥇" : idx === 1 ? "🥈" : idx === 2 ? "🥉" : `${idx + 1}`; return <tr key={staff.id} className="border-b border-white/5 hover:bg-white/5 transition-colors"><td className="py-3.5 pr-4 text-center font-bold text-sm">{rankEmoji}</td><td className="py-3.5 px-3"><div className="font-semibold text-white/95">{staff.name}</div><div className="text-[10px] text-white/45">ID: {staff.staffId} · {staff.branch || "Unknown Branch"}</div></td><td className="py-3.5 px-3 text-right font-medium text-white/70">฿{Math.round(target).toLocaleString()}</td><td className="py-3.5 px-3 text-right font-bold text-white">฿{Math.round(actual).toLocaleString()}</td><td className="py-3.5 px-3 text-center"><span className={`px-2 py-0.5 rounded font-bold text-[10px] ${achPercent >= 100 ? "bg-emerald-500/20 text-emerald-400" : "bg-white/10 text-white/70"}`}>{achPercent.toFixed(1)}%</span></td><td className="py-3.5 px-3 text-right font-medium text-white/60">฿{Math.round(forecast).toLocaleString()}</td><td className="py-3.5 px-3 text-center"><span className={`font-semibold ${coverPlusRate >= 25 ? "text-emerald-400" : "text-white/60"}`}>{coverPlusRate}%</span></td><td className="py-3.5 px-3 text-center"><span className={`font-semibold ${acRate >= 20 ? "text-emerald-400" : "text-white/60"}`}>{acRate}%</span></td><td className="py-3.5 px-3 text-center"><span className={`font-semibold ${penRate >= 85 ? "text-emerald-400" : "text-white/60"}`}>{penRate}%</span></td><td className="py-3.5 px-3 text-center"><span className={`font-semibold ${caseRate >= 60 ? "text-emerald-400" : "text-white/60"}`}>{caseRate}%</span></td><td className="py-3.5 px-3 text-center"><span className="font-bold text-teal-300">{simUnits} Unit</span></td></tr>; })}</tbody></table></div></div>;
}
